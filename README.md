...
4. (Part 1) Deployed several T-Pot instances in different locations around the globe to harvest location specific data. Deployment locations: MontrÈal, S„o Paulo, Los Angeles, Frankfurt and Tokyo
5. (Part 2) Deployed a VM named ìHoneyCoreî to aggregate all of the data from the different VM instances deployed in part 2 via Logstash to harvest data representing attacks on a global scale. Deployment locations: Taiwan, Tokyo, Sydney, Frankfurt, Z¸rich, MontrÈal, Iowa and Salt Lake City
6.  Harvested data from Kibana for both parts of the project


## Results

In the first part of the project, statistics were compared from honeypots stationed in five different locations: MontrÈal, S„o Paulo, Los Angeles, Frankfurt and Tokyo over a twenty-four hour period. Across all of these honeypots, the Dionaea honeypot was the most attacked, with the Cowrie honeypot being the second most attacked. The honeytrap honeypot was the third most attacked across almost all locations, with the exception being that the honeypot stationed in MontrÈal had the heralding honeypot as its third most attacked honeypot (see Figure 1).
...
The results from p0f in which p0f was able to identify the operating system being run on the machines of attackers showed that most of the machines were running Windows 7 or 8. Destination ports that were repeatedly seen to be attacked were 445, 443, 1433 and 8088. The main reputation that most of the source IP addresses had was ìknown attackerî, for all of the deployments (see Figure 4 for an example).

<img src="/data_visualization/fig_4.png"/>

Figure 4: Attacker Source IP Reputation for Tokyo Deployment  

In the second part of the project, the data from even more VM instances (different from those in part one) was aggregated into one core instance, HoneyCore. The individual instances were situated in Taiwan, Tokyo, Sydney, Frankfurt, Z¸rich, MontrÈal, Iowa and Salt Lake City.

The main honeypots attacked over these aggregated instances were the same as in the first part of the project: dionaea, cowrie, honeytrap, rdpy and heralding, which was also one of the top five attacked honeypots in S„o Paulo in the first part (see Figure 5). The total number of attacks recorded on the HoneyCore deployment over a 24 hour window amounted to more than 1.3 million attacks. 
...
There were two major attack periods over 24 hours, in which Suricata alert categories of the type ìAttempted User Privilege Gainî spiked. 386,333 attacks of this nature occurred at 21:30 and 443,932 attacks of that same type took place at 00:30 (as seen in Figure 7).  
...
Overall there were more than 1.3 million attacks over a 24 hour period in our core deployment alone. This offers a mere glance at how many attacks take place daily on a global scale as well as the importance of following proper security practices such as patching up security vulnerabilities as soon as one is detected so that they cannot be exploited. The arsenal of tools for attackers is expected to diversify more over time as the security measures taken by users intensify. With virtualization and the cloud steadily becoming a major part of most organizationís infrastructure, attacks and potential security mitigations are only going to get more complex. Honeypots offer helpful oversights and can act as a passive monitor that allows administrators to keep up to date with the methods used by modern day attackers. As attackers evolve, so must the precautions taken by the defenders.

## References

[1] Paliwal, Savita. ‚ÄúHoneypot: A Trap for Attackers.‚Äù International Journal of Advanced Research in Computer and Communication Engineering, vol. 6, no. 3, Mar. 2017, pp. 842‚Äì845., doi:10.17148/ijarcce.2017.63197.  
[2] Paralax. ‚ÄúParalax/Awesome-Honeypots.‚Äù GitHub, 14 Dec. 2019, github.com/paralax/awesome-honeypots   
[3] Dtag-Dev-Sec. ‚ÄúDtag-Dev-Sec/Tpotce.‚Äù GitHub, 22 Apr. 2020, github.com/dtag-dev-sec/tpotce  
[4]  ‚Äú4.1. Rules Format.‚Äù 4.1. Rules Format - Suricata 4.1.0-Dev Documentation, 2016, https://suricata.readthedocs.io/en/suricata-4.1.4/rules/intro.html  
[5] Zalewski, Michal. ‚Äúp0f v3 (Version 3.09b).‚Äù p0f v3, lcamtuf.coredump.cx/p0f3/  
[6] ‚ÄúDocumentation.‚ÄùDionaea, dionaea.carnivore.it/documentation/  
[7] Public Safety Canada. ‚ÄúSamba (Smbd) Vulnerability.‚Äù AV16-060, 4 May 2016, www.publicsafety.gc.ca/cnt/rsrcs/cybr-ctr/2016/av16-060-en.aspx  
[8] ‚ÄúSecurity Updates and Information.‚Äù Samba, www.samba.org//samba/history/security.html  
[9] Ismanto, Melvin. ‚ÄúHow to Use EternalBlue to Exploit SMB Port Using Public Wi-Fi.‚Äù Medium, Medium, 18 June 2019, medium.com/@melvinshb/how-to-use-eternalblue-to-exploit-smb-port-using-public-wi-fi-79a996821767  
[10] Microsoft Support.‚ÄùMicrosoft Security Advisory: Vulnerability in SQL Server could allow remote code execution‚Äù..‚ÄùSupport.microsoft.com, support.microsoft.com/en-ca/help/961040/microsoft-security-advisory-vulnerability-in-sql-server-could-allow-re  
[11] ‚ÄúMS09-004 Microsoft SQL Server sp_replwritetovarbin Memory Corruption via SQL Injection.‚Äù Rapid7, www.rapid7.com/db/modules/exploit/windows/mssql/ms09_004_sp_replwritetovarbin_sqli  
[12] ‚ÄúWhat Is SQL Injection? Tutorial & Examples: Web Security Academy.‚Äù What Is SQL Injection? Tutorial & Examples | Web Security Academy, portswigger.net/web-security/sql-injection  
[13] Manuel, Jasper, et al. ‚ÄúOMG: Mirai-Based Bot Turns IoT Devices into Proxy Servers.‚Äù Fortinet Blog, 21 Feb. 2018, www.fortinet.com/blog/threat-research/omg--mirai-based-bot-turns-iot-devices-into-proxy-servers.html  
[14] Trost, Jason. ‚ÄúAdventures with Heralding, a Credential Grabbing Honeypot.‚Äù Covert.io, 15 Dec. 2016, www.covert.io/adventures-with-heralding-a-credential-grabbing-honeypot/  
[15] KacyConnect, Kacy Zurkus News WriterEmail. ‚ÄúMalicious Login Attempts Spike in Finance, Retail.‚Äù Infosecurity Magazine, 20 Sept. 2018, www.infosecurity-magazine.com/news/malicious-login-attempts-spike-in/  
[16] Vasilyev, Igor. ‚Äúclassification.config.‚Äù Classification.config - Suricata - Open Information Security Foundation, 26 July 2018, https://redmine.openinfosecfoundation.org/attachments/1529  
[17] Team, SecurityTrails. ‚ÄúSecurityTrails: Cybersecurity Fingerprinting Techniques and OS-Network Fingerprint Tools.‚Äù Cybersecurity Fingerprinting: What Is It? Fingerprint Techniques and Tools, SecurityTrails, 21 May 2019, securitytrails.com/blog/cybersecurity-fingerprinting  
